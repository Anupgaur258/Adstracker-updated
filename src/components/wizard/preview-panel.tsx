...
interface PreviewPanelProps {
  /** Which layer to highlight / animate. Others show static. */
  activeLayer?: "hook" | "cta" | "subtitle" | "all";
  /** Override hook index for preview (default: first filled) */
  hookIndex?: number;
  /** Override cta index for preview (default: first filled) */
  ctaIndex?: number;
  children?: React.ReactNode;
}

export function PreviewPanel({ activeLayer = "all", hookIndex, ctaIndex, children }: PreviewPanelProps) {
    ...
    <div className="rounded-xl border border-border p-3 space-y-3 bg-card/50">
      <PhonePreview screenColor="black">
        <div className="relative w-full h-full">
          ...
          {showHook && (
            <div
              className="absolute left-0 right-0 px-3 transition-opacity"
              style={{
                top: `${wizardState.styling.hookYPosition}%`,
                transform: "translateY(-50%)",
                textAlign: wizardState.styling.hookXPosition,
                opacity: hookOpacity,
              }}
            >
              <p
                className={cn(
                  "font-semibold leading-tight inline-block max-w-full break-words",
                  activeLayer === "hook" && `overlay-anim-${hookTmpl.animation}`
                )}
                style={{
                  color: hookColor,
                  fontFamily: hookFont,
                  fontSize: `${Math.min(hookFontSize * 0.45, 16)}px`,
                  ...
                  ...(hookOutlineColor !== "transparent" && hookOutlineWidth > 0 && {
                    WebkitTextStroke: `${hookOutlineWidth * 0.5}px ${hookOutlineColor}`,
                  }),
                }}
              >
                {hookText}
          ...
          {showSub && (
            <div
              className="absolute left-0 right-0 px-3 transition-opacity"
              style={{
                top: `${wizardState.styling.subtitleYPosition}%`,
                transform: "translateY(-50%)",
                textAlign: wizardState.styling.subtitleXPosition,
                opacity: subOpacity,
              }}
            >
              <p
                className={cn(
                  ...
                  activeLayer === "subtitle" && animClass[previewSubtitle!.animation]
                )}
                style={{
                  fontFamily: previewSubtitle!.fontFamily,
                  fontSize: "10px",
                  color: previewSubtitle!.color,
                  backgroundColor: previewSubtitle!.backgroundColor !== "transparent" ? previewSubtitle!.backgroundColor : undefined,
                  padding: previewSubtitle!.backgroundColor !== "transparent" ? "2px 6px" : undefined,
                  borderRadius: 3,
          ...
          {showCta && (
            <div
              className="absolute left-0 right-0 px-3 transition-opacity"
              style={{
                top: `${wizardState.styling.ctaYPosition}%`,
                transform: "translateY(-50%)",
                textAlign: wizardState.styling.ctaXPosition,
                opacity: ctaOpacity,
              }}
            >
              <span
                className={cn(
                  ...
                  activeLayer === "cta" && `overlay-anim-${ctaTmpl.animation}`
                )}
                style={{
                  ...ctaStyleFn(ctaColor),
                  fontFamily: ctaFont,
                  fontSize: `${Math.min(ctaFontSize * 0.55, 13)}px`,
                  ...(ctaBoxColor !== "transparent" && {
                    backgroundColor: ctaBoxColor,
                    borderRadius: 4,
                  }),
                  ...(ctaOutlineColor !== "transparent" && ctaOutlineWidth > 0 && {
                    WebkitTextStroke: `${ctaOutlineWidth * 0.5}px ${ctaOutlineColor}`,
                  }),
                }}
              >
                {ctaText}
